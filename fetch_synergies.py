...
The build page loads teammates from the mega API when you click
Normalised Synergy Delta. That JSON is already sorted into role lists
with headers [id, wr, d1, d2, pr, n]. This pulls `d2` for every pairing.
"""
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Download LoLalytics normalised synergy Δ2 pairings to CSV."
    )
    parser.add_argument("--tier", default="emerald_plus")
    parser.add_argument(
        "--patch",
        default="30",
        help="LoLalytics patch query. 30 = last 30 days, 14 = last 14 days.",
    )
    parser.add_argument(
        "--lane",
        default="all",
        help="Main champion lane sent to the API. 'all' matches the site default.",
    )
